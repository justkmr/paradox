    ...
    <section id="projects" className="w-full bg-transparent relative z-10">
      <div className="w-full flex flex-col">
        ...
        <div className="max-w-7xl mx-auto w-full px-4 md:px-8 pt-24 md:pt-44 pb-12 select-none text-left relative flex flex-col md:flex-row md:items-end md:justify-between gap-6 md:gap-8">
          ...
          <div className="flex flex-wrap gap-1.5 font-mono text-[11px] md:text-[12px] bg-black/10 p-2 border border-white/10 rounded-[18px] backdrop-blur-md self-start md:self-auto max-w-full overflow-x-auto scrollbar-none">
            {filterTabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveFilter(tab.id as FilterType)}
                className={`px-3 md:px-4 py-2 rounded-[15px] uppercase tracking-widest font-bold transition-all duration-300 whitespace-nowrap ${
                  activeFilter === tab.id
                    ? "text-[#E24C60] bg-[#E24C60]/10 border border-[#E24C60]/20 shadow-[0_0_20px_rgba(226,76,96,0.05)]"
                    : "text-zinc-500 hover:text-[#E24C60] border border-transparent"
                }`}
              ...
              return (
                ...
                >
                  ...
                  <div className="max-w-7xl mx-auto w-full px-4 md:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center">
                    {/* 🖼️ RIGHT COLUMN: Terminal Mockup */}
                    <div className="lg:col-span-6 w-full order-1 lg:order-2 group/vessel">
                      <div className="w-full bg-[#0c0d0f]/60 backdrop-blur-md border border-white/5 rounded-[24px] md:rounded-[28px] overflow-hidden shadow-[0_40px_90px_-25px_rgba(0,0,0,0.95)] group-hover/vessel:border-[#E24C60]/20 transition-all duration-700 flex flex-col">
                        <div className="w-full bg-black/40 px-5 md:px-6 py-3.5 border-b border-white/5 flex items-center justify-between font-mono select-none">
                          <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2">
                              <div className="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-white/5 border border-white/10 group-hover/vessel:bg-[#E24C60]/40 transition-colors duration-500" />
                              <div className="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-white/5 border border-white/10" />
                              <div className="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-white/5 border border-white/10" />
                            </div>

                            <span className="text-[10px] md:text-[11px] text-[#E24C60] tracking-widest flex items-center gap-1.5 border-l border-white/10 pl-3 md:pl-4">
                              <FontAwesomeIcon
                                icon={faSquare}
                                className="text-[10px] text-[#E24C60]/60"
                              />
                              RENDER_TARGET // PORT_0{index + 1}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center justify-center h-full">
                            {project.featured && (
                              <div className="flex items-center gap-1.5 bg-[#E24C60]/10 border border-[#E24C60]/30 rounded-full px-2.5 py-1 text-[8px] md:text-[12px] text-[#E24C60] font-bold uppercase tracking-widest scale-[0.95] animate-pulse my-auto transform translate-y-[0.5px]">
                                <FontAwesomeIcon
                                  icon={faStar}
                        ...
                        <div className="p-4 md:p-6 bg-black/30 border-t border-white/5 flex flex-wrap gap-2 font-mono">
                          {project.tech.map((t) => (
                            <div
                              key={t}
                              className="flex items-center gap-1.5 text-zinc-500 py-1 px-2.5 bg-white/2 border border-white/5 rounded-[12px] hover:text-[#E24C60] hover:bg-[#E24C60]/5 hover:border-[#E24C60]/20 transition-all duration-300 select-none"
                            >
                              <span className="w-1 h-1 bg-zinc-600 group-hover/vessel:bg-[#E24C60] rounded-full transition-colors duration-500" />
                    ...
                    <div className="lg:col-span-6 text-left flex flex-col justify-center order-2 lg:order-1">
                      <div className="flex items-center gap-4 font-mono select-none">
                        ...
                        <span className="text-[14px] md:text-[15px] text-gray-400 tracking-wider font-semibold">
                          {project.subtitle}
                        </span>
                      </div>

                      <h3 className="text-5xl md:text-6xl lg:text-7xl font-display font-black text-transparent bg-clip-text bg-linear-to-b from-white via-white to-zinc-500 uppercase tracking-tight mt-4 md:mt-6 leading-none">
                        {project.title}
                      </h3>

                      <p className="mt-1 md:mt-2 text-base md:text-base text-zinc-400 font-serif font-light leading-relaxed tracking-wide max-w-xl">
                        {project.description}
                      </p>

                      <div className="mt-5 border-t border-b border-white/5 py-4 md:py-5 grid grid-cols-3 gap-2 md:gap-4 font-mono text-[14px] md:text-[15px] select-none">
                        <div className="flex flex-col gap-1">
                          <span className="text-zinc-600 tracking-wider flex items-center gap-1.5 uppercase font-medium text-[11px] md:text-[14px]">
                            <FontAwesomeIcon
                              icon={faCalendarDays}
                              className="text-[#E24C60] text-lg md:text-[15px]"
                            />
                            DATE_STAMP
                          </span>
                          <span className="text-zinc-400 tracking-wide truncate">
                            ...
                            typeof project.date === "object" ? (
                              ...
                              (project.date as unknown as string) || "N/A"
                            )}
                          </span>
                        </div>

                        <div className="flex flex-col gap-1 border-l border-white/5 pl-3 md:pl-4">
                          <span className="text-zinc-600 tracking-wider flex items-center gap-1.5 uppercase font-medium text-[11px] md:text-[14px]">
                            <FontAwesomeIcon
                              icon={faUserTie}
                              className="text-[#E24C60] text-lg md:text-[15px]"
                      ...
                      <div className="flex items-center justify-between gap-4 mt-8 pt-5  border-white/5 font-mono text-[10px] md:text-[11px] tracking-widest font-bold w-full">
                        {project.githubLink && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-zinc-300 hover:text-white transition-all duration-300 flex items-center gap-2 group/btn relative bg-zinc-900/50 border border-white/5 rounded-[12px] py-2.5 px-4 md:px-5 shadow-lg shrink-0"
                          >
                            <FontAwesomeIcon
                              icon={faGithub}
                              className="text-sm group-hover/btn:rotate-12 transition-transform duration-300"
                            />
                            <span>GET_SOURCE</span>
                            <span className="absolute bottom-0 left-0 w-0 h-px bg-white group-hover/btn:w-fit transition-all duration-300" />
                          </a>
                        )}
                        {project.liveLink && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-zinc-300 hover:text-[#E24C60] transition-all duration-300 flex items-center gap-1.5 group/live relative bg-zinc-900/50 border border-white/5 rounded-[12px] py-2.5 px-4 md:px-5 shadow-lg shrink-0 ml-auto"
                          >
                            <span>LIVE_EXECUTE</span>
                            <FontAwesomeIcon
                              icon={faArrowUpRightFromSquare}
                              className="text-[10px] group-hover/live:translate-x-0.5 group-hover/live:-translate-y-0.5 transition-transform duration-300"
                            />
                            <span className="absolute bottom-0 right-0 w-0 h-px bg-[#E24C60] group-hover/live:w-25 transition-all duration-300" />
                          </a>
                        )}
                      </div>
                    </div>
